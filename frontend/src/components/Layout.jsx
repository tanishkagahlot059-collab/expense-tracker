...
import { ArrowDown, ArrowUp, Activity, Car, CreditCard, DollarSign, Gift, Home, PiggyBank, ShoppingCart, TrendingUp, Utensils, Zap, RefreshCw, Clock, Info, ChevronUp, ChevronDown, PieChart } from 'lucide-react';
...
const filterTransactions = (transactions, frame) => {
 ...
 const safeArrayFormResponse = (res) => {
    const body = res?.data;
    if (!body) return [];
...
const Layout = ({ onLogout, user }) => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const incomes = safeArrayFormResponse(incomeRes).map((i) => ({
        ...i,
        type: "income",
      }));
      const expenses = safeArrayFormResponse(expenseRes).map((e) => ({
        ...e,
        type: "expense",
      }));
            ...
            <div className={styles.cards.base}>
              <div className={styles.cards.header}>
                <h3 className={styles.cards.title}>
                  ...
                  Financial Overview
                  <span className=" text-sm text-gray-500 font-normol">
                    ({timeFrameLabel})
                  </span>
                </h3>
              </div>
